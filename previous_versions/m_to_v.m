% Load the data
folderPath = 'D:\Popovic\Anja\EEG-emotion\EEG-emotion';

% Get all .mat files in the folder
matFiles = dir(fullfile(folderPath, 'connectivity_matrices_*.mat'));
...
for i = 1:numel(sortedMatFiles)
    ...
    % Reshape the connectivity matrix to a 2D matrix (assuming 32x32xnum_of_epochs)
    reshapedMatrix = reshape(conG2_epochs, [], size(conG2_epochs, 3));
