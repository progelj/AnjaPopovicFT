...
%filename='';
filepath= 'D:\Popovic\Anja\EMOTIONS 1';
...
order = 10;
numDatasets = length(OUTEEG); % OUTEEG is an array of EEG datasets
conG2_all_datasets = cell(1, numDatasets);

tic

for datasetIndex = 1:numDatasets
    currentEEG = OUTEEG(datasetIndex); % OUTEEG is an array of EEG datasets
    ...
    save(['connectivity_matrices' num2str(datasetIndex)], 'conG2_epochs', 'epochLabels');
...
for epoch = 1:numEpochs
    ...
    colorbar;
    title(['Epoch ' num2str(epoch)], 'FontSize', 25);
    xticks([1:32]);
    % Convert channel labels to a cell array of strings
    channelLabels = cellstr(EEG.chanlocs.labels(:));
    ax = gca;
    ...
    xlabel('Influencing electrode','FontSize', 25);
    ylabel('Influenced electrode', 'FontSize', 25);
    axis equal;
    axis tight;
end
